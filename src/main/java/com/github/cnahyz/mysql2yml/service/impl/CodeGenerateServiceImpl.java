...
import com.github.cnahyz.mysql2yml.entity.Callback;
import com.github.cnahyz.mysql2yml.entity.SaveFile;
import com.github.cnahyz.mysql2yml.entity.TableInfo;
import com.github.cnahyz.mysql2yml.entity.Template;
import com.github.cnahyz.mysql2yml.service.CodeGenerateService;
import com.github.cnahyz.mysql2yml.service.SettingsStorageService;
import com.github.cnahyz.mysql2yml.service.TableInfoSettingsService;
import com.github.cnahyz.mysql2yml.tool.*;
...
public class CodeGenerateServiceImpl implements CodeGenerateService {
     ...
     */
    @Override
    public void generate(Collection<Template> templates, TableOptions tableOptions) {
        // 获取选中表信息
        TableInfo selectedTableInfo;
        List<TableInfo> tableInfoList;
        if (Boolean.TRUE.equals(tableOptions.getEntityModel())) {
        ...
        // 如果使用统一配置，直接全部覆盖
        if (Boolean.TRUE.equals(tableOptions.getUnifiedConfig())) {
            tableInfoList.forEach(tableInfo -> {
                tableInfo.setSaveModelName(finalSelectedTableInfo.getSaveModelName());
                tableInfo.setSavePackageName(finalSelectedTableInfo.getSavePackageName());
                tableInfo.setSavePath(finalSelectedTableInfo.getSavePath());
        ...
        // 生成代码
        generate(templates, tableInfoList, tableOptions, null);
    }
     ...
     * @param tableOptions  生成配置
     * @param otherParam    其他参数
     */
    public void generate(Collection<Template> templates, Collection<TableInfo> tableInfoList, TableOptions tableOptions, Map<String, Object> otherParam) {
        ...
        for (TableInfo tableInfo : tableInfoList) {
            ...
            // 设置额外代码生成服务
            param.put("generateService", new ExtraCodeGenerateUtils(this, tableInfo, tableOptions));
            for (Template template : templates) {
                Callback callback = new Callback();
                callback.setWriteFile(true);
                callback.setReformat(tableOptions.getReFormat());
                // 默认名称
                ...
                new SaveFile(project, code, callback, tableOptions).write();
            }
        }
    }
